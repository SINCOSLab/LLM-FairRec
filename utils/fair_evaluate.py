...
def ranking_evaluate(user_emb, item_emb, n_users, n_items, train_u2i, test_u2i, sens=None, indicators='[\'ndcg\',\'precision\']',
                     topks='[1, 5, 10, 20]', num_workers=4):
...
def eval_accelerate(scores, n_users, train_u2i, test_u2i, indicators, topks, num_workers):
    ...
    #     res.append(test_one_perf1(u,scores, train_u2i, test_u2i, indicators, topks))
...
def test_one_perf1(u,scores, train_u2i, test_u2i, indicators, topks):
    u_id = u
    score = np.copy(scores[u_id])
    uid_train_pos_items = list(train_u2i[u_id])
    uid_test_pos_items = list(test_u2i[u_id])
    score[uid_train_pos_items] = -np.inf
    score_indices = largest_indices(score, topks)
    res1 = get_perf1(score_indices, uid_test_pos_items, topks,indicators)
    res2 = get_topks_items1(u_id, score_indices,topks)
    return (res1, res2)
...
def get_perf1(rank, uid_test_pos_items, topks,indicators):
    topk_eval = np.zeros(len(indicators) * len(topks), dtype=np.float32)
    k = 0
    for ind in indicators:
        for topk in topks:
...
def get_topks_items1(uid, rank,topks):
    max_topk = max(topks)
    topk_items = rank[:max_topk]
    return np.hstack([np.array(uid, dtype=np.int32), topk_items])
